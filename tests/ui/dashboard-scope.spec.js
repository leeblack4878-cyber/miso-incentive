...
test('매장 선택은 핵심 성과·예상 마감·직원 목록을 함께 바꾼다',async({page},info)=>{
 ...
 const hs=page.getByTestId('admin-metric-hs');await expect(hs).toContainText('30건');await expect(hs).toContainText('예상 마감 60건');
 await page.getByLabel('운영 현황 매장').selectOption(stores[0]);await expect(hs).toContainText('10건');await expect(hs).toContainText('예상 마감 20건');
 await page.getByLabel('운영 현황 매장').selectOption(stores[1]);await expect(hs).toContainText('20건');await expect(hs).toContainText('예상 마감 40건');
 ...
 await page.getByLabel('운영 현황 매장').selectOption('all');await expect(hs).toContainText('예상 마감 60건');
});
test('매장 관리자는 본인 매장만 보고 마감 확정 실적을 재환산하지 않는다',async({page})=>{
 ...
 await expect(page.getByTestId('admin-metric-hs')).toContainText('12건');await expect(page.getByTestId('admin-metric-hs')).toContainText('예상 마감 12건');
});
test('영업담당 선택 목록은 담당 상권 안에서만 제공한다',async({page})=>{
 ...
 await picker.selectOption(stores[1]);await expect(page.getByTestId('admin-metric-hs')).toContainText('예상 마감 40건');
});
