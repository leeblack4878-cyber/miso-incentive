...
test('store type determines thresholds, personal counts reprice eligible sales',()=>{
 ...
 assert.equal(p.mobilePay,30000);assert.equal(p.internetPay,160000);assert.equal(p.tvPay,20000);assert.equal(p.total,210000);
});
test('date bounds, saved sale not counted twice, new sale uses resulting personal tier',()=>{
 const p=chuseokPolicy({store_name:'월곶점',store_mobile:99,mobile_count:4,store_internet:0,internet_count:0,store_tv:0,tv_count:0});
