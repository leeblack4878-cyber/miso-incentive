import {readAppSource} from './helpers/app-source.js';
...
test('판매 수정 중 여러 약속을 다시 저장해도 task_meta를 null로 보내지 않는다', async () => {
  const source = await readAppSource();
  assert.match(source, /return rows\.map\(row=>\(\{\.\.\.row,task_meta:row\.task_meta\|\|\{\}\}\)\)/);
...
test('신규 판매는 정책 스냅샷을 저장하고 수정 시 기존 정책을 유지한다', async () => {
  const source = await readAppSource();
  assert.match(source, /policySnapshot:editingSale\?\.source_meta\?\.policySnapshot\|\|currentPolicySnapshot\(config\)/);
...
test('직원 홈 설치 처리 내역은 완료와 취소를 별도로 조회한다', async () => {
  const source = await readAppSource();
  const homeManager = await readFile(new URL('../src/components/HomeOrderManager.jsx',import.meta.url),'utf8');
...
test('직원 오늘 할 일은 항목별 화면과 필터로 바로 이동한다', async () => {
  const source = (await Promise.all(['../src/App.jsx','../src/components/CustomerCareManager.jsx','../src/components/TodayWorkCard.jsx','../src/components/SpecialSalePolicyAdmin.jsx'].map(path=>readFile(new URL(path,import.meta.url),'utf8')))).join('\n');
...
test('정책 달력과 DB 기준 스냅샷으로 지난달 지급기준을 보존한다', async () => {
  ...
  assert.match(app, /policySnapshot:editingSale\?\.source_meta\?\.policySnapshot\|\|currentPolicySnapshot\(config\)/);
...
test('지원 판매는 선택 매장 팀 실적에만 반영하고 개인 계산에서 제외한다', async () => {
  ...
  assert.match(source, /const credit=activeTeamSupport\?\{credited_store:teamSupportStore,metrics:mobileTeamCreditMetrics/);
