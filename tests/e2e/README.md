...
2026-09-16 전용 staging `ahmvrflklvmdprolauuy`에서 실제 Auth 로그인/Chromium/DB 4개 모두 통과.
수정 커밋 `bb43389`, GitHub Actions run `35104998203`.
최초 run `35104258039`는 3/4 통과: pending 묶음 재저장 시 약속 소실을 검출했다.
첫 판매만 보던 편집 조회를 묶음 전체로 수정했고, 약속과 비용을 서로 다른 판매 행에
연결한 fixture로 재발을 검출한다. 단위·소스 계약/모킹 UI가 아닌 실제 인증 테스트다.
테스트 fixture 후처리까지 통과했으며 운영 DB에는 테스트하지 않았다.
동일 저장소 소유자 PR에서 관련 경로가 변경되면 자동 실행하며 fork PR은 제외한다.
