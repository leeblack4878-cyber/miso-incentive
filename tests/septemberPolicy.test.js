...
test('특가&지인정책은 요금제와 전략P를 모두 충족해야 추가 지급한다', () => {
  ...
  assert.equal(calculateSeptemberSpecialSale({ policyKey: 'f971_256_mnp', planGroup: '115', strategicPoints: 2, saleDate: '2026-09-15' }).eligible, false);
  assert.equal(calculateSeptemberSpecialSale({ policyKey: 'f971_256_mnp_0915', planGroup: '115', strategicPoints: 2, saleDate: '2026-09-14' }).eligible, false);
  assert.equal(calculateSeptemberSpecialSale({ policyKey: 'f971_256_mnp_0915', planGroup: '115', strategicPoints: 2, saleDate: '2026-09-15' }).additionalAmount, 50000);
  assert.equal(calculateSeptemberSpecialSale({ policyKey: 'f971_256_change_0915', planGroup: 'youth85', strategicPoints: 2, saleDate: '2026-09-15' }).additionalAmount, 50000);
});
