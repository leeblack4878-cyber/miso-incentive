...
export function friendlyError(err) {
  ...
  if (/sale_.*mismatch|sale_invalid_payload/.test(lower)) return '저장 결과를 확인하지 못했어요. 새로고침 후 다시 확인해주세요.';
  if (/home_write_forbidden|sale_write_forbidden/.test(lower)) return '이 내역을 처리할 권한이 없어요. 관리자에게 문의해주세요.';
  ...
  if (lower.includes('is_month_locked') || lower.includes('month_locked')) {
