...
export default function SalesExpensePanel({ userId, month, onTotal, won, fmtInputNumber }) {
  ...
  const add=async()=>{
    ...
    if(!form.expense_date||form.expense_date.slice(0,7)!==month)return showAppToast('선택한 월 안에서 비용 날짜를 입력해주세요.',{tone:'error'});
    ...
    setForm(f=>({...f,amount:'',customer_name:'',memo:''}));await load();
  ...
  return <div className="surface-card overflow-hidden">
    <button onClick={()=>setOpen(v=>!v)} className="w-full p-4 flex justify-between items-center text-left">
      ...
      <span className="text-xs text-brand-600">{open?'접기':'등록/내역'}</span>
    </button>
    {open&&<div className="px-4 pb-4 space-y-3">
      <div className="grid grid-cols-2 gap-2">
        <input aria-label="비용 날짜" type="date" value={form.expense_date} onChange={e=>setForm({...form,expense_date:e.target.value})} className="border rounded-lg px-2 py-2 text-xs"/>
