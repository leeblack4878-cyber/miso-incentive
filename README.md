...
- `tests/db/sale-integrity.sql`: 비어 있는 전용 스테이징에서 실행하는 실제 SQL/RLS 회귀. 가상 데이터·오류 주입 트리거·월 마감 설정은 모두 ROLLBACK합니다.
...
정책 금액·적용월·권한 범위는 임의로 변경하지 않습니다. 10월에 9월 정책이 이어지는 현재 동작은 표시만 명확히 했으며 새 정책을 만들어 적용하지 않았습니다.
